...
import { addLogEntry } from "./utils/logging";
import { enforceTabLimit } from "./utils/tabManager";
import { updateAllWindowBadges } from "./utils/badge";
import { handleMessage } from "./utils/messageHandler";
import { getWindowCount } from "./utils/windowManager";
import { showWindowLimitAlertAndClose } from "./utils/windowLimitAlert";
    ...
    if (windowCount > config.windowLimit && config.autoCloseWindows) {
      ...
      setTimeout(async () => {
        try {
          ...
          console.error(
            "Tab Monitor: Error showing window limit alert:",
            error,
          );
        }
      }, 500);
